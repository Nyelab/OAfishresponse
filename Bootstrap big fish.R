...
#we (Nye et al) looked at these ones, namely bootstrapped all big fish vs all small fish 
#data1<-data[data$Size_group=="small",]
# data1<-data[data$Size_group=="big",]
...
#For large fish sample sizes are more even so don't need to subsample Acantho.
#Just pull all fish that aren't small
#If you do it this way there are 10 fish that aren't classified into a group
#So we just kept the designations of Clark et al

#data1<-data[data$Size_group!="small",]

data1<-data[data$Size_group=="medium"|data$Size_group=="big",]
...
#see range of sizes, all over 15mm
range(data1$SL_mm,na.rm=TRUE)
...
## The Shapiro Wilk tests above indicate non-normality even those the W value is high, histograms looks good
## There appear to be a few "extreme" values that make the sample not normal at the tails looking at qq plots
## Shapiro Wilk is sensitive to extreme values
## Conducted a ttest of the raw data to test if the distributions between control and treatment fish are different
## The default in R is to to the Welch test which corrects for uneven variances by changing the df
## To assume even variances turn this off with var.equal=TRUE in the second line of code
## If the t values are similar between the two tests the distribution is close to normal 
...
#I leave commented out until I've checked output
#write.table(boot.results,"bigfish_bootresults.csv",row.names=F,sep=",",quote=F) 
